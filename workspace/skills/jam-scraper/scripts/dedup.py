"""
JAM dedup — fix and remove duplicate price history data.

jam_properties has asset_id as PK (confirmed stable provider ID), so no
...
from models import JamPriceHistory
...
def dedup_price_history(session: Session, dry_run: bool) -> int:
    result = session.execute(text("""
        ...
        GROUP BY asset_id, DATE(scraped_at), change_type
        HAVING COUNT(*) > 1
        ORDER BY cnt DESC
    """)).fetchall()

    if not result:
        print("jam_price_history: no duplicates found")
        return 0
    ...
    if not dry_run and delete_ids:
        session.execute(delete(JamPriceHistory).where(JamPriceHistory.id.in_(delete_ids)))
...
def main(apply: bool) -> None:
    dry_run = not apply
    print(f"=== JAM dedup [{'DRY RUN' if dry_run else 'APPLYING'}] — {datetime.now().isoformat()} ===\n")
    ...
    print(f"\nSummary: jam_price_history rows {'would be' if dry_run else ''} deleted: {deleted}")
    if dry_run and deleted:
        print("Re-run with --apply to execute.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deduplicate JAM price history")
